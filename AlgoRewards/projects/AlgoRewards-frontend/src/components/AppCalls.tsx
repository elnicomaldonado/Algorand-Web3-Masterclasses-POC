...
const AppCalls = ({ openModal, setModalState }: AppCallsInterface) => {
  ...
  const sendAppCall = async () => {
    ...
    // For TestNet, we'll use the already deployed contract
    // App ID: 743651454 (deployed to TestNet)
    const appClient = new AlgoRewardsContractFactory({
      defaultSender: activeAddress ?? undefined,
      algorand,
    }).getAppClientById({ appId: BigInt(743652051) })
